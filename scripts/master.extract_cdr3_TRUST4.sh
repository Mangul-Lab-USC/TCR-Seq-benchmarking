...
ls raw_data/complete_sample/MIXCR_Data/*.fastq.sort._report.tsv | cut -d "/" -f4 | awk -F ".fastq.sort._report.tsv" '{print $1}' >TRUST_samples.txt

while read line
do
echo "python scripts/TRUST4.extract.py raw_data/complete_sample/TRUST4_Data/${line}.fastq.sort._report.tsv raw_data/complete_sample/TRUST4_Data/extracted_features/${line}.extracted_TRUST4.csv"
