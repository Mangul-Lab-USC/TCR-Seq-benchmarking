...
ls raw_data/complete_sample/MIXCR_Data/*.fastq.sort.cdr3 | cut -d "/" -f4 | awk -F ".fastq.sort.cdr3" '{print $1}' >IMREP_samples.txt

while read line
do
echo "python scripts/imrep.extract.py raw_data/complete_sample/IMREP_Data/${line}.fastq.sort.cdr3 raw_data/complete_sample/IMREP_Data/extracted_features/${line}.extracted_IMREP.csv"
